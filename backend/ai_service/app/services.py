...
def query_model(query):
    ...
    db = FAISS.load_local(
        "faiss_index", knowledge_embeddings, allow_dangerous_deserialization=True
    )
...
def getEmbeddings():
    CHOSEN_LITE_LLM_EMBEDDING_MODEL = (
        "Amazon Titan - Bedrock Text Embedding v2 (Internal)"
    )
    litellm_proxy_endpoint = os.environ.get(
        "litellm_proxy_endpoint", "https://gq8rdw5man.us-east-1.awsapprunner.com"
    )
...
def generatePlots(query):
    ...
    prompt = PromptTemplate(
        template="""You are a chart generation LLM who given some data and a task. 
            ...
            When you are creating the various other graphs make sure you are getting the correct data, for example if you are making a graph about comparing different companies you grab the same datapoint from each company.
            Task: {task}
            Data: {data} \n \n 
            Object:
            ```json """,
...
def generateMatPlotLibCode(query):
    ...
    knowledge_embeddings = getEmbeddings()
    db = FAISS.load_local(
        f"faiss_index", knowledge_embeddings, allow_dangerous_deserializations
    )
...
def agent_organizer(query):
    prompt = PromptTemplate(
        template="""
            ...
            Answer with 'Graphs' if the user is asking you to generate different plots or graphs. ex: pie_chart, bar_chart, metric, bullet_point, or graphs in general
    ...
    if ans == "Graphs":
        ...
        similarities = [
            cosine_similarity([query_vector], [prompt_vector])[0][0]
            for prompt_vector in prompt_embeddings
        ]
